...
def obtener_clientes(id_cliente, cursor):
    ...
    try:
        ...
        if id_cliente == 'todos':  # Si no se recibe un id
            ...
            cursor.execute('SELECT idCliente, nombre, apellidoPaterno, apellidoMaterno, fechaNacimiento, sexo, estadoCivil, calle, numeroExterior, numeroInterior, colonia, codigoPostal, referencia, correo FROM cliente WHERE idCliente = %s', (id_cliente,))  # Se ejecuta una consulta SQL con un parámetro
...
def registrar_cliente(body, cursor, conexion):
    ...
    try:
        ...
        cursor.execute('INSERT INTO cliente VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (body['telefono'], body['nombre'].upper(), body['apellidoPaterno'].upper(), body['apellidoMaterno'].upper(), body['fechaNacimiento'], body['sexo'].upper(), body['estadoCivil'].upper(), body['calle'].upper(), body['numeroExterior'], body['numeroInterior'], body['colonia'].upper(), body['codigoPostal'], body['referencia'].upper(), body['correo'], contrasenia_encriptada))
        conexion.connection.commit()  # Se confirma la transacción
        return jsonify({'success': True, 'status': 201, 'message': 'El cliente se ha registrado exitosamente', 'data': {'nombre': body['nombre'].upper(), 'apellidoPaterno': body['apellidoPaterno'].upper(), 'apellidoMaterno': body['apellidoMaterno'].upper(), 'fechaNacimiento': body['fechaNacimiento'], 'sexo': body['sexo'].upper(), 'estadoCivil': body['estadoCivil'].upper(), 'calle': body['calle'].upper(), 'numeroExterior': body['numeroExterior'], 'numeroInterior': body['numeroInterior'], 'colonia': body['colonia'].upper(), 'codigoPostal': body['codigoPostal'], 'referencia': body['referencia'].upper(), 'telefono': body['telefono'], 'correo': body['correo']}})  # Se retorna un objeto JSON con un mensaje de éxito
...
def actualizar_cliente(id_cliente, cursor, conexion):
    ...
    try:
        ...
        contrasenia_encriptada = generate_password_hash(cliente['contrasenia'], method='pbkdf2:sha256')
        cursor.execute('SELECT idCliente FROM cliente WHERE idCliente = %s', (id_cliente,))
        if cursor.fetchone() is not None:
            cursor.execute('UPDATE cliente SET nombre = %s, apellidoPaterno = %s, apellidoMaterno = %s, fechaNacimiento = %s, sexo = %s, estadoCivil = %s, calle = %s, numeroExterior = %s, numeroInterior = %s, colonia = %s, codigoPostal = %s, referencia = %s, correo = %s, contrasenia = %s WHERE idCliente = %s', (cliente['nombre'].upper(), cliente['apellidoPaterno'].upper(), cliente['apellidoMaterno'].upper(), cliente['fechaNacimiento'], cliente['sexo'].upper(), cliente['estadoCivil'].upper(), cliente['calle'].upper(),cliente['numeroExterior'], cliente['numeroInterior'],cliente['colonia'].upper(), cliente['codigoPostal'], cliente['referencia'].upper(),cliente['correo'], contrasenia_encriptada, id_cliente,))
            if cliente['telefono'] != id_cliente:
                cursor.execute('UPDATE cliente SET idCliente = %s WHERE idCliente = %s', (cliente['telefono'], id_cliente,))
            ...
            return jsonify({'error': {'code': 404, 'type': 'Error del cliente', 'message': 'Cliente no encontrado', 'details': f'El cliente {id_cliente} no existe'}}) # Se retorna un objeto JSON con un error 500    
    except IntegrityError as e:
        return jsonify({'error': {'code': 400, 'type': 'Error del cliente', 'message': 'Error de integridad MySQL', 'details': str(e)}}) # Se retorna un objeto JSON con un error 500
...
def eliminar_clientes(id_cliente, cursor, conexion):
    ...
    try:
        ...
        if cursor.fetchone() is not None:
            ...
            conexion.connection.commit()
            return jsonify({'success': True, 'status': 200, 'message': f'El cliente {id_cliente} se ha dado de baja exitosamente'})  # Se retorna un objeto JSON con un mensaje de éxito
        else:
            return jsonify({'error': {'code': 404, 'type': 'Error del cliente', 'message': 'Cliente no encontrado', 'details': f'El cliente {id_cliente} no existe'}}) # Se retorna un objeto JSON con un error 404
    except OperationalError as e:
        return jsonify({'error': {'code': 500, 'type': 'Error del servidor', 'message': 'Error en la base de datos', 'details': str(e)}}) # Se retorna un objeto JSON con un error 500
